...
一方で、書いたコードが実際のゲームで正しく動くかの確認は、いまだに人間が Play ボタンを押して目視しています。
ここを任せられないのは AI の能力の問題ではなく、ゲーム側に AI が操作・観測するための入り口がないからです。

私は最近、この入り口になる Unity 用のデバッグコンソール / コマンド実行ライブラリ **LiminalPalette** を開発し、活用しています。
